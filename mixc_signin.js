//2026/06/13  //16:08
/*
@Name：一点万象app 自动化签到 (多账号版)
@Author：怎么肥事 (Surge多账号适配)

使用说明：
1. 依次登录不同账号，进入签到页，脚本会自动捕获每个账号的参数
2. 每个账号只需登录一次，之后自动签到
3. 最多支持 10 个账号
*/
...
const STORE_KEY = "mixc_signin_accounts";
...
function getAccountId(cfg) {
  // 用 token 后几位 + mallNo 组合成简短ID
  const tokenShort = cfg.token ? cfg.token.slice(-6) : "unknown";
  return tokenShort + "_" + (cfg.mallNo || "nomall");
}
...
function doCapture() {
  ...
  // 生成账号标识
  const accountId = getAccountId(saved);
  saved._id = accountId;
  ...
  for (let i = 0; i < accounts.length; i++) {
    if (accounts[i]._id === accountId || accounts[i].token === saved.token) {
      existingIdx = i;
      break;
    }
  }
  ...
  if (existingIdx >= 0) {
    ...
    if (accounts.length >= MAX_ACCOUNTS) {
      notify("万象星多账号", "账号已达上限 ❌", "最多 " + MAX_ACCOUNTS + " 个，请删除旧账号");
      passthrough();
      return;
    }
    accounts.push(saved);
  ...
  writeStore(accounts);
  notify("万象星多账号", isNew ? "新账号已添加 ✅" : "账号已更新 ✅", 
         "共 " + accounts.length + " 个账号 · " + (saved.mallNo || ""));
  passthrough();
}
...
function signOneAccount(cfg, index, total, callback) {
  const ms = Date.now();
  const p = {
  ...
  httpPost("https://app.mixcapp.com/mixc/gateway", headers, buildBody(p), function (err, body) {
    let result = { index: index, total: total, success: false, message: "", points: "" };
    
    if (err) {
      result.message = "请求异常: " + err;
      callback(result);
      return;
    }
...
function doSign() {
  ...
  if (!accounts || accounts.length === 0) {
    notify("万象星多账号", "未抓到任何账号 ❌", "请先登录App进入签到页");
    finish();
    return;
  }
  ...
  notify("万象星多账号", "开始签到 " + total + " 个账号 ⏳", "请稍候...");
  
  for (let i = 0; i < total; i++) {
    const cfg = accounts[i];
    const idx = i + 1;
    
    signOneAccount(cfg, idx, total, function(result) {
      results.push(result);
      completed++;
      
      // 每条结果单独通知
      const label = "账号" + result.index + "/" + result.total;
      if (result.success) {
        notify("万象星·" + label, "✅ " + result.message, "");
        successCount++;
      } else {
        notify("万象星·" + label, "❌ " + result.message, "");
      }
